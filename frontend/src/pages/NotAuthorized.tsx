...
export default function NotAuthorizedPage() {
      ...
      const isAlreadyLoggedIn = location.state?.login || false;

      const handleAction = () => {
            if (role === 'admin'){
                  window.location.href = '/admin/login'
            }else{
                  window.location.href = isAlreadyLoggedIn ? '/products' : '/home';
            }
      };
      ...
      return (
            <div className="fixed inset-0 flex flex-col items-center justify-center bg-gray-50 dark:bg-gray-900 p-6">
                  <div className="text-center">
                        ...
                        <button onClick={handleAction} className="px-6 py-2 bg-blue-600 hover:bg-blue-700 text-white font-normal text-sm rounded-sm transition">
                              ← Go Back
                        </button>
                  </div>
            </div>
